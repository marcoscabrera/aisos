<template>
    <div class="VJS_video-player-wrapper" v-on:keydown.space="togglePlayPause" tabindex="0">
        ...
        <video class="VJS_video-player" ref="videoPlayer" v-on:click="togglePlayPause" :preload="props.preload">
            <source v-for="source in props.sources" :src="source.url" :type="source.type"  :key ="source.id" />
            ...
            <div class="VJS_controls__bar">
                <div ref="progressBar" class="VJS_controls__progress" v-on:click="skipToPosition($event)">
                    ...
                    <div class="VJS_controls__progress-ranges">
                        <span class="VJS_controls__progress-range" v-for="range in bufferRanges" :key="range.id" v-bind:style="{ left: range.start, width: range.end }"></span>
                    </div>
                </div>
...
<script>
        ...
        return {
            props: {
                ...
                validator: function(prop) {
                    ...
                    if(!prop.['sources'] || !prop.sources.length || !Array.isArray(prop.sources)) {
                    ...
                    for(let i=0, l=prop.sources.length; i<l; i++) {
                        for(let j=0, k=sourceValues.length; j<k; j++) {
                            if(!prop.sources[i][sourceValues[j])) {
    ...
    export default {
        ...
        mounted: function() {
           ...
           // typeof this.videoControls;
           // typeof this.progressLoop;
            // Vars
            vm = this;
